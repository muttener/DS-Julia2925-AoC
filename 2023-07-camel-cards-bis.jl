...
function number_of_j(h::Hand)
    length(filter( (x -> x == _J),h))
end

function score_hand(h::Hand)
    js = number_of_j(h) # CHANGE
    a = sort(group(filter((x -> x != _J),h)), by=(x->x[2]),rev=true)
    if isempty(a) # five of a kind with J
        return 7
    else 
        first = a[1][2] + js # CHANGE
        if first == 5 # five of a kind
            return 7
        elseif first == 4 # four of a kind
            return 6
        elseif first == 3
            if a[2][2] == 2
                return 5 # full house
